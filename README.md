...
- [Create an **OpenAI API key**](https://platform.openai.com/api-keys) for **OpenAI Transcription API** transcription or **GPT API** translation. Enter it in the corresponding transcription or translation field.
...
- **Input**: [**yt-dlp**](https://github.com/yt-dlp/yt-dlp) extracts audio from live streams; local media files and PC device audio are also supported.
- **Audio slicing**: dynamic-threshold slicing based on [**Silero-VAD**](https://github.com/snakers4/silero-vad).
- **Transcription**: locally with [**Whisper**](https://github.com/openai/whisper) / [**Faster-Whisper**](https://github.com/SYSTRAN/faster-whisper) / [**SimulStreaming**](https://github.com/ufal/SimulStreaming) / [**HuggingFace ASR**](https://huggingface.co/models?pipeline_tag=automatic-speech-recognition), or remotely with the [**OpenAI Transcription API**](https://platform.openai.com/docs/guides/speech-to-text).
- **Translation** (optional): OpenAI's [**GPT API**](https://platform.openai.com/docs/overview) or Google's [**Gemini API**](https://ai.google.dev/gemini-api/docs).
- **Output**: print to **terminal**, save to a **file** (e.g. **.srt** subtitles), or send to **Discord** / **Telegram** / **Cqhttp**.
...
To select the WebUI interface language at startup, use `--ui-language` with `en`, `zh`, or `ja`.
...
- **OpenAI Transcription API**: ```stream-translator-gpt {URL} --language {input_language} --use-openai-transcription-api --openai-transcription-api-key {your_openai_transcription_key}```
...
| `--openai-transcription-api-key`   |                                | OpenAI API key for OpenAI Transcription API. If omitted, `--openai-api-key` is used. If you have multiple keys, you can separate them with "," and each key will be used in turn.                                                                                  |
| `--openai-transcription-base-url`  |                                | Customize the API endpoint of OpenAI Transcription API.                                                                                                                                                                                                               |
| `--openai-transcription-model`     | gpt-transcribe                 | OpenAI transcription model. `gpt-transcribe` is recommended for ordinary transcription; `whisper-1`, `gpt-4o-mini-transcribe`, and `gpt-4o-transcribe` are also supported.                                                                                            |
...
| `--transcription-keywords`         |                                | Comma-separated transcription keywords. `gpt-transcribe` sends them as `keywords`; other Whisper backends use them as an initial prompt. The deprecated `--transcription-initial-prompt` alias is still accepted.                                                     |
| `--transcription-context`          |                                | Pass the previous transcription result as context; only one result is retained. SimulStreaming and HuggingFace ASR do not support text context (disabled by default).                                                                                                 |
| **Translation Options**            |
...
| `--openai-api-key`                 |                                | OpenAI API key for GPT translation. If you have multiple keys, you can separate them with "," and each key will be used in turn.                                                                                                                                     |
| `--openai-base-url`                |                                | Customize the API endpoint of OpenAI for GPT translation.                                                                                                                                                                                                              |
| `--gpt-model`                      | gpt-5.6-luna                   | OpenAI's GPT model name, gpt-5.4-nano / gpt-5.4-mini / gpt-5.6-luna / gpt-5.6-terra                                                                                                                                                                                   |
| `--google-api-key`                 |                                | Google API key for Gemini translation. If you have multiple keys, you can separate them with "," and each key will be used in turn.                                                                                                                                  |
| `--google-base-url`                |                                | Customize the API endpoint of Google for Gemini translation.                                                                                                                                                                                                          |
...
| `--translation-history-size`       | 3                              | The number of previous transcripts sent as context when calling the LLM API. It is recommended to disable context (set to 0) for weaker models.                                                                                                                       |
...
| `--reasoning-effort`               | auto (lowest supported)        | GPT parameter. Controls reasoning depth for reasoning models. Options: none / minimal / low / medium / high / xhigh / max.                                                                                                                                            |
| `--verbosity`                      |                                | GPT parameter. Controls the verbosity of the response. Options: low / medium / high.                                                                                                                                                                                  |
| `--service-tier`                   |                                | GPT parameter. Specifies processing priority tier. Options: auto / default / flex / priority.                                                                                                                                                                         |
| `--extra-body`                     |                                | A string containing a JSON object with additional top-level fields for the GPT-compatible API request body, e.g. `--extra-body "{\"enable_thinking\": false}"`.                                                                                                                  |
