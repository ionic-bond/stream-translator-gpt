...
class SimulLLM:
    ...
    def __init__(self, llmtrans, min_len=0, chunk=1, trimming="sentences", language="ja", init_src="", init_tgt=""):
        ...
        self.buffer = ParallelTextBuffer(self.llmtranslator.tokenizer,
                                         self.llmtranslator.max_tokens_to_trim,
                                         trimming=trimming,
                                         init_src=init_src,
                                         init_tgt=init_tgt)
...
parser.add_argument('--input-instance',
                    type=str,
                    default=None,
...
parser.add_argument('--min-len', type=int, default=1, help='Minimum number of space-delimited words at the beginning.')
...
parser.add_argument('--init_prompt_src',
                    type=str,
                    default=None,
...
parser.add_argument('--init_prompt_tgt',
                    type=str,
                    default=None,
...
parser.add_argument('--len-threshold',
                    type=float,
                    default=None,
...
lan_thresholds = {
    'de':
        1.3,  # 12751/9817  ... the proportion of subword tokens for ACL6060 dev de vs. en text, for EuroLLM-9B-Instruct tokenizer
    'ja': 1.34,  # 13187/9817
    'zh': 1.23,  # 12115/9817
...
parser.add_argument('--language-specific-len-threshold',
                    default=False,
                    action="store_true",
...
simul = SimulLLM(llmtrans,
                 language=lan,
                 min_len=args.min_len,
                 chunk=args.min_chunk_size,
                 init_src=init_src,
                 init_tgt=init_tgt,
                 trimming=args.buffer_trimming)
    ...
    def yield_input():
        for line in sys.stdin:
            ...
            if text[0] != " ":
                ...
                yield (ts, beg, end, " " + first
                      )  # marking the first word with " ", so that it can be later detected and inserted as suffix
            else:
                words = text.split()
            for w in words:
                yield (ts, beg, end, w)
